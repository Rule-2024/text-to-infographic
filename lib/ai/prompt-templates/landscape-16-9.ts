...
在【核心尺寸规格】部分新增：
"HTML文档结构要求：
  * 必须包含完整HTML5文档结构：<!DOCTYPE html>声明
  * 严格遵循<html><head><body>层级结构
  * head部分必须包含：<title>标签、<meta charset>声明
  * body部分必须使用语义化容器标签"
...
2. CSS内联样式规范：
  * 所有样式必须内联在<style>标签中
  * 使用CSS变量定义设计系统参数
  * 尺寸单位统一使用px（打印场景）或vw/vh（响应式场景）
  * 包含打印媒体查询 @media print {}
...
- **安全区域控制**：
  * 四周30px安全边距，重要内容不得超出此范围
  * 核心内容区域限定在1860px × 1020px内
