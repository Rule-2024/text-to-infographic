    ...
    const checkStatus = async () => {
      try {
        ...
        if (elapsedTime > maxProcessingTime) {
          console.log(
            `Processing timeout after ${elapsedTime}ms for size: ${sizeParam || 'unknown'}`
          );

          // 根据尺寸提供更具体的错误信息
          if (sizeParam === '16-9') {
            setError(
              'Processing the 16:9 landscape format is taking longer than expected. This format requires more processing power. Please try again with a shorter text or switch to the mobile format for faster results.'
            );
          } else if (sizeParam === 'a4-l') {
            setError(
              'Processing the A4 landscape format is taking longer than expected. This format requires more processing power. Please try again with a shorter text or switch to the mobile format for faster results.'
            );
          } else if (sizeParam === 'a4-p') {
            setError(
              'Processing the A4 portrait format is taking longer than expected. This format requires more processing power. Please try again with a shorter text or switch to the mobile format for faster results.'
            );
          } else {
            setError(
              'Processing is taking longer than expected. Please try again with a shorter text or different settings.'
            );
          }
          return;
        }
        ...
        if (data.status === 'completed') {
          ...
          if (data.error) {
            ...
            if (sizeParam === '16-9') {
              setError(
                'Generation of 16:9 landscape format failed. This format is more complex and may require shorter text input. Please try again with less text or switch to the mobile format.'
              );
            } else if (sizeParam === 'a4-l') {
              setError(
                'Generation of A4 landscape format failed. This format is more complex and may require shorter text input. Please try again with less text or switch to the mobile format.'
              );
            } else if (sizeParam === 'a4-p') {
              setError(
                'Generation of A4 portrait format failed. This format is more complex and may require shorter text input. Please try again with less text or switch to the mobile format.'
              );
            } else {
              setError(
                'Generation failed, please try again with a shorter text or different settings.'
              );
            }
          }
          return;
    ...
    return (
      <main className="flex min-h-screen flex-col items-center justify-center p-4 sm:p-8 md:p-24 bg-gradient-to-b from-background to-muted">
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          ...
          ></div>
          <div
            className="absolute top-1/4 right-1/4 w-40 h-40 bg-secondary/5 rounded-full blur-2xl animate-pulse"
            style={{ animationDelay: '1.5s' }}
          ></div>
          <div
            className="absolute bottom-1/4 left-1/4 w-40 h-40 bg-primary/5 rounded-full blur-2xl animate-pulse"
        ...
        <div className="glass-card p-8 card-shadow max-w-md text-center relative z-10">
          ...
          <p className="mb-6 text-sm text-muted-foreground">
            Don't worry! This happens occasionally when our AI is processing complex content. Let's
            try again with a few adjustments to get better results.
          </p>
      ...
      <div className="glass-card p-8 card-shadow max-w-md text-center relative z-10">
        <div className="mb-6 relative">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            <span className="inline-flex items-center">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="h-3 w-3 mr-1 text-secondary"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
              ...
              Estimated time:{' '}
              {progress < 30 ? '60-90 seconds' : progress < 60 ? '30-60 seconds' : '15-30 seconds'}
            </span>
          </p>
        </div>
        ...
        <div className="mb-6 text-left bg-background/50 rounded-lg p-4 border border-border/50">
          ...
          <p className="text-sm text-muted-foreground">
            ...
            {progress >= 30 &&
              progress < 60 &&
              'Designing the optimal layout for your information...'}
            {progress >= 60 &&
              progress < 90 &&
              'Creating visual elements and applying design styles...'}
        ...
        <div className="mb-6 text-left bg-background/50 rounded-lg p-4 border border-border/50">
          ...
          <p className="text-xs text-muted-foreground">
            {progress < 30 &&
              'Infographics are processed 60,000 times faster by the human brain than text alone. Your content is being transformed for maximum impact!'}
            {progress >= 30 &&
              progress < 60 &&
              "Color choices in infographics can increase willingness to read by up to 80%. We're selecting the perfect palette for your content."}
            {progress >= 60 &&
              progress < 90 &&
              'People remember 80% of what they see, but only 20% of what they read. Your infographic will make your message more memorable!'}
            {progress >= 90 &&
              'High-quality infographics are 30 times more likely to be read than plain text articles. Your content is almost ready to shine!'}
          </p>
        </div>
        ...
        <div className="mb-6 text-center">
          <p className="text-sm font-medium text-primary animate-pulse">
            {progress < 50
              ? 'Please wait while we craft your infographic...'
